...
| NPC manifest format | Dialogue only — the NPC's current voice written as if to the reader (first person, note-style, or direct address). No third-person description. Followed by a bare hex hash on its own line at the bottom. No label on the hash. The hash is each NPC's identity fingerprint — when it changes, the NPC has been corrupted. The player is not told this explicitly until they've already witnessed it. |
