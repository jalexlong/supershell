...
| NPC manifest format | `Name:` on the first line as a speaker label, followed by the NPC's dialogue (first person, direct address — no third-person description). Bare hex hash on its own line at the bottom. No label on the hash. Reads like a JRPG dialogue box in a text file. The hash is each NPC's identity fingerprint — when it changes, the NPC has been corrupted. The player is not told this explicitly until they've already witnessed it. |
