...
fn default_title() -> String { "Untitled Course".to_string() }
    ...
    pub fn check(&self, user_command: &str, state: &GameState) -> ValidationResult {
        let is_valid = match &self.condition_type {
            ConditionType::CommandMatches { pattern } => {
                ...
                if re.is_match(user_command) { true } else { return ValidationResult::SyntaxError; }
            }
            ...
            ConditionType::FileNotContains { path, pattern } => {
                if let Ok(content) = fs::read_to_string(Self::get_sandbox_path(path)) {
                    Regex::new(pattern).map(|re| !re.is_match(&content)).unwrap_or(true)
                } else { true }
            }
            ConditionType::FileEmpty { path } => {
                 fs::metadata(Self::get_sandbox_path(path)).map(|m| m.len() == 0).unwrap_or(false)
            }
            // --- ENV CHECKS ---
            ConditionType::WorkingDir { path } => {
                let current = env::current_dir().unwrap_or_default().to_string_lossy().to_string();
                Regex::new(path).map(|re| re.is_match(&current)).unwrap_or(false)
            }
            ConditionType::EnvVar { name, value } => env::var(name).map(|v| v == *value).unwrap_or(false),
        ...
        if is_valid {
            ...
            let msg = self.failure_message.clone()
